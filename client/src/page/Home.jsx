...
import { Bot, Camera, Zap, Cpu, HardDrive, Share2 } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-black font-['Inter',sans-serif] selection:bg-accent selection:text-white relative overflow-hidden">
      <style>
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-8 pt-12 pb-24 grid lg:grid-cols-2 gap-20 items-center">
        <div className="space-y-8 ">
            ...
            <p className="text-lg text-text-muted max-w-md mt-10">Real-time computer vision for school workshops. Monitor PPE compliance and station availability with one simple system.</p>
            <button onClick={() => handleNavigateToDashboard()} className="bg-accent text-white px-8 py-4 rounded-xl shadow-xl shadow-accent/25 hover:scale-105 transition-transform">Launch Dashboard</button>
        </div>
      ...
      <section className="py-24 bg-[#D9DDDC] border-t border-border pb-27">
        <div className="max-w-7xl mx-auto px-8">
          <div className="mb-16">
            <h2 className="text-sm uppercase tracking-[0.3em] text-accent mb-4">Workflow</h2>
            <h3 className="text-4xl font-bold tracking-tight">The 3-Step Safety Loop</h3>
          </div>

          <div className="grid md:grid-cols-3 gap-12 relative">
            {/* Connection line (visible on desktop) */}
            <div className="hidden md:block absolute top-1/4 left-0 w-full h-px bg-linear-to-r from-transparent via-border to-transparent -z-10"></div>
      ...
      <section className="py-24 bg-surface-muted border-y border-border">
        <div className="max-w-7xl mx-auto px-8 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <h2 className="text-sm font-black uppercase tracking-[0.3em] text-accent mb-4">Hardware</h2>
            <h3 className="text-4xl font-extrabold tracking-tight mb-6">Built for the Workshop.</h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="aspect-square bg-white rounded-3xl border border-border flex flex-col items-center justify-center p-8 text-center group hover:border-accent transition-colors">
               <Cpu className="w-10 h-10 text-accent mb-4 group-hover:scale-110 transition-transform" />
               <span className="text-xs font-black uppercase tracking-widest text-text-muted">Processor</span>
               <span className="text-sm font-bold">Arm Cortex-A72</span>
            </div>
            <div className="aspect-square bg-white rounded-3xl border border-border flex flex-col items-center justify-center p-8 text-center group hover:border-accent transition-colors">
               <HardDrive className="w-10 h-10 text-accent mb-4 group-hover:scale-110 transition-transform" />
               <span className="text-xs font-black uppercase tracking-widest text-text-muted">Storage</span>
               <span className="text-sm font-bold">Supabase DB</span>
            </div>
            <div className="col-span-2 bg-ink rounded-3xl p-8 flex items-center justify-between text-white border border-white/5 shadow-2xl shadow-accent/10">
               <div>
               ...
               <div className="bg-accent p-3 rounded-xl"><Bot className="w-6 h-6" /></div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* --- Footer --- */}
      <footer className="py-16 text-center text-text-muted bg-white">
...
function WorkflowStep({ number, title, desc, icon }) {
  return (
    <div className="relative p-8 bg-surface-muted border border-border rounded-2xl hover:border-accent transition-all group">
      ...
      <div className="mb-6 w-12 h-12 bg-white rounded-xl border border-border flex items-center justify-center group-hover:shadow-lg group-hover:shadow-accent/10 transition-all">{icon}</div>
...
function HardwareItem({ title, detail }) {
  return (
    <div className="flex items-start gap-4">
      <div className="mt-1.5 w-2 h-2 rounded-full bg-accent"></div>
      <div>
        <h5 className="font-bold text-sm uppercase tracking-tight">{title}</h5>
        <p className="text-xs text-text-muted font-medium">{detail}</p>
      </div>
    </div>
  );
}